  ...
  return (
    <div className="bg-gray-100 min-h-screen pb-10">
      <div className="px-4 md:px-10 py-4 text-sm text-gray-500">
        <Link to="/" className="hover:underline">Home</Link> {'>'} {product.category} {'>'} {product.name}
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex gap-2 overflow-x-auto">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="w-16 h-16 border border-gray-300 rounded p-1 cursor-pointer hover:border-blue-500">
                  <img src={mainImage} className="w-full h-full object-contain" alt="thumbnail" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:flex-1">
            ...
            <div className="mt-4 space-y-3 text-sm border-b pb-5">
              <div className="flex"><span className="w-24 text-gray-400">Category:</span><span>{product.category}</span></div>
              <div className="flex"><span className="w-24 text-gray-400">Description:</span><span>{product.description}</span></div>
            </div>
          </div>
          ...
          <div className="lg:w-64 border border-gray-200 rounded-md p-4 shadow-sm h-fit">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div><p className="text-sm font-medium">Supplier</p><p className="text-xs text-gray-500">Global Trade Corp</p></div>
            </div>
            <div className="space-y-2 text-xs text-gray-500 mb-4 border-t pt-3">
              <div className="flex gap-2"><img src={FlagGER} className="w-4 h-3" alt="flag" /> Germany, Berlin</div>
              <p>✓ Verified Seller</p>
